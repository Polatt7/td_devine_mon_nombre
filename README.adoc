...
.Quelles commandes avez-vous tapées ?
....
🡆 git add src && git commit -m "mon message" && git push 🡄
....
...
+
....
🡆 &mut permet à read_line de modifier la variable input pour y mettre la saisie de l’utilisateur. 🡄
....
.. Quel est le type de retour de `read_line` ? Quel est son utilité ?
+
....
🡆 Result<usize> indique si la lecture a réussi ou échoué, et le usize correspond au nombre d’octets lus. 🡄
....
...
+
....
🡆 cargo init a créé Cargo.toml, .gitignore et src/main.rs. 🡄
....
